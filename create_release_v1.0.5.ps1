# Script to create GitHub release v1.0.5
# Prerequisites: GitHub CLI must be installed and authenticated

Write-Host "Creating GitHub Release v1.0.5..." -ForegroundColor Green
...
if (-not (Test-Path $latestJsonPath)) {
    ...
    Write-Host "Make sure you've run: .\update_latest_json.ps1" -ForegroundColor Yellow
    exit 1
}
...
$renamedInstaller = "BetSure.Dialer_1.0.5_x64-setup.exe"
Copy-Item $installerPath $renamedInstaller -Force

try {
    gh release create v1.0.5 `
        --title "v1.0.5 - Call Activities Fix & Rate Limit Handling" `
        --notes-file $releaseNotesPath `
        "$renamedInstaller" `
        "$latestJsonPath"
    
    Write-Host "`n✅ Release created successfully!" -ForegroundColor Green
    Write-Host "Release URL: https://github.com/Visionatedigital/bet-wise-dialer/releases/tag/v1.0.5" -ForegroundColor Cyan
