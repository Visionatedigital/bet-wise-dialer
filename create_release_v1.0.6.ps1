# Script to create GitHub release v1.0.6
# Prerequisites: GitHub CLI must be installed and authenticated

Write-Host "Creating GitHub Release v1.0.6..." -ForegroundColor Green
...
$renamedInstaller = "BetSure.Dialer_1.0.6_x64-setup.exe"
Copy-Item $installerPath $renamedInstaller -Force

try {
    gh release create v1.0.6 `
        --title "v1.0.6 - Version 6 Release" `
        --notes-file $releaseNotesPath `
        "$renamedInstaller" `
        "$latestJsonPath"
    
    Write-Host "`n✅ Release created successfully!" -ForegroundColor Green
    Write-Host "Release URL: https://github.com/Visionatedigital/bet-wise-dialer/releases/tag/v1.0.6" -ForegroundColor Cyan
