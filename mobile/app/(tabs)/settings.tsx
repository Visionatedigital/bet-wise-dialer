...
// Lazy-load ImagePicker to prevent native module errors from blanking the screen
let ImagePicker: typeof import("expo-image-picker") | null = null;
try {
  ImagePicker = require("expo-image-picker");
} catch {
  // ImagePicker not available – photo upload will be disabled
}
...
export default function SettingsScreen() {
  ...
  const processAndUpload = async (result: any) => {
  ...
  const roleLabel = user?.role?.toUpperCase() ?? "AGENT";
  ...
  return (
    ...
    >
      ...
      <View style={styles.avatarSection}>
        ...
        <Text style={styles.userName}>{user?.full_name || user?.email || "My Account"}</Text>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <Feather name="log-out" size={16} color={colors.status.error} style={{ marginRight: 8 }} />
...
const styles = StyleSheet.create({
  ...
  avatarSection: { alignItems: "center", paddingTop: 32, paddingBottom: 12 },
  avatarImg: {
  ...
  cameraBtn: {
    ...
    elevation: 3, borderWidth: 1, borderColor: colors.border.default,
  },
  userName: {
    fontSize: 17, fontWeight: "700", color: colors.text.primary,
    marginTop: 10,
  },
  avatarHint: { fontSize: 12, color: colors.text.secondary, marginTop: 4 },
  card: {
